...
function unique<Type>(array: Type[]): Type[] {
    const seen = new Set<Type>(); // Use a Set to track unique values
...
function testCssClasses(parsed: ParsedOptions, entry: CssClasses): void {
    ...
    if (typeof parsed.cssPrefix === "string") {
        ...
        Object.keys(entry).forEach((key) => {
            const typedKey = key as keyof CssClasses; // Explicitly cast key
            parsed.cssClasses[typedKey] = parsed.cssPrefix + entry[typedKey];
        });
    } else {
...
function testOptions(options: Options): ParsedOptions {
    ...
    const tests: { [key in keyof Options]: { r: boolean; t: (parsed: ParsedOptions, entry: unknown|any) => void } } = {
...
data.listeners.forEach(function (c: [string, EventHandler]) {
    const eventType = c[0] as keyof HTMLElementEventMap;  // Ensures c[0] is a valid event type
    const listener = c[1] as EventListener;  // Cast EventHandler to EventListener

    scope_DocumentElement.removeEventListener(eventType, listener);
});
    ...
    function destroy(): void {
        ...
        Object.keys(options.cssClasses).forEach((key) => {
            const typedKey = key as keyof CssClasses; // Explicitly cast key
            removeClass(scope_Target, options.cssClasses[typedKey]);
        });
